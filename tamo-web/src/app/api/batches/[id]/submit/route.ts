...
export async function POST(_: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await authenticatedMediaRequest(credential, `batches/${encodeURIComponent(id)}/submit`, { method: 'POST' });
  ...
  } catch { return NextResponse.json({ title: 'Unable to submit batch', status: 503 }, { status: 503 }); }
}
