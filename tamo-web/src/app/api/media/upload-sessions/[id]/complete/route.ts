...
export async function POST(_: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await authenticatedMediaRequest(credential, `upload-sessions/${encodeURIComponent(id)}/complete`, { method: 'POST' });
    ...
    return response;
  } catch { return NextResponse.json({ title: 'Upload unavailable', status: 503 }, { status: 503 }); }
}
