...
describeIntegration('authentication lifecycle integration',()=>{
  ...
  it('consumes an email-verification token only once',async()=>{const pending=await repository.create({email:`pending-${randomUUID()}@example.test`,firstName:'Pending',lastName:'Test',role:AccountRole.BUYER,passwordHash:'not-used',registrationNoticeVersion:'2026-09-09'});const hash=randomBytes(32);await repository.createVerificationToken(pending.id,hash,new Date(Date.now()+60_000));await expect(repository.consumeVerificationToken(hash)).resolves.toBe(true);await expect(repository.consumeVerificationToken(hash)).resolves.toBe(false);await pool.query('DELETE FROM users WHERE id=$1',[pending.id]);});
